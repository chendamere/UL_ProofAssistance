...
while(expi < beginexp.length){
    ...
    if(!pf.isRule(r)){
        ...
        let tempstack
        if(next == -1) {
            let x = provefromstack(expi, p,stackindex)
            if(x[0] != -1){
                fromstack = true
                next = x[0]
                tempstack=x[1]
        ...
        while(next != -1 && !pf.Same(npr.leftexps, npr.rightexps)){
            if(fromstack){
                ...
                if(e[0] != -1){
                    ...
                    let x = provefromstack(expi, next, stackindex)
                    // console.log('*&*&', x )
            ...
            if(next == -1) {
                ...
                // let check = pf.Operands_normalize(pf.genRule('!'+pf.ExpToString(npr.leftexps) +'@'+endexp[expi]+'\n')[0])
                // console.log('goal? ', pf.RuleToString(goal))
                // console.log(pf.ExpToString(npr.leftexps),pf.ExpToString(npr.rightexps))

                // if(pf.Same(npr.leftexps,npr.rightexps) && ! pf.Same(goal.leftexps, goal.rightexps)){
                //     console.log(pf.ExpToString(npr.leftexps),pf.ExpToString(npr.rightexps))
